...
extern int yylex();
extern int yyparse();
void function_def(int func_type, char *name, int enter, Node *stmt), variable(int constant, Node * varType, char *name, Node *init);
void externs(int public, int func, int var,char *name), add_void_func(char *name);
int yyerror(char *s);
...
file:									
	| file error ';'
	| file public tipo ID ';'			{ IDnew($3->value.i, $4, 0); externs($2, 0, 1, $4); declare($2, 0, $3, $4, 0); variable(0,$3,$4,0);}
	| file public CONST tipo ID ';'		{ IDnew($4->value.i+5, $5, 0); externs($2, 0, 1, $5); declare($2, 1, $4, $5, 0); variable(1 ,$4, $5,0);}
	| file public tipo ID init			{ IDnew($3->value.i, $4, 0); declare($2, 0, $3, $4, $5); variable(0,$3,$4,$5); }
	| file public CONST tipo ID init	{ IDnew($4->value.i+5, $5, 0); declare($2, 1, $4, $5, $6); variable(1,$4, $5, $6); }
	| file public tipo ID 				{ enter($2, $3->value.i, $4); } finit { function($2, $3, $4, $6); if(fwdFlag) {externs(0, 1, 0, $4); fwdFlag=0;} else function_def($3->value.i, $4, -pos, $6); }
	| file public VOID ID 				{ enter($2, 4, $4); voidFlag=1; add_void_func($4); } finit { function($2, intNode(VOID, 4), $4, $6); if(fwdFlag){ externs(0, 1, 0, $4); fwdFlag=0;} else function_def(4,$4, -pos, $6);}
	;
...
finit: '(' {pos = 8; } params ')' 	{ if (voidFlag == 1) {pos = 0; voidFlag=0;} else pos=funcPos;} 	blocop 	{ $$ = binNode('(', $6, $3); }
	|  '(' {pos = 0;} ')' blocop        																	{ $$ = binNode('(', $4, nilNode(NIL)); }
	;

blocop  : ';'   					{ $$ = nilNode(NIL);fwdFlag=1; IDpop();}
        | bloco ';'   				{ $$ = $1; IDpop();}
        ;
...
param: tipo ID               	{ $$ = binNode(PARAM, $1, strNode(ID, $2));
                                  if (pos > 0) {
                                  	...
                                  	if($1->value.i % 5 < 3 ) pos +=4;
                                  	else if ($1->value.i % 5 == 3) pos += 8;
                                  }
                                  if (pos <= 0) {
                                  	if($1->value.i % 5 < 3 ) pos -=4;
                                  	else if ($1->value.i % 5 == 3) pos -= 8;
...
void enter(int pub, int typ, char *name) {
	/* public type->value.i (a number corresponding to the type) ID */
...
{	/* public tipo ID finit */
	Node *bloco = LEFT_CHILD(body);    /*left_child body if empty = 0*/
	/*if(popFlag != 1) 
		IDpop();*/
	if (bloco != 0) { /* not a forward declaration */
		long par;										
		int fwd = IDfind(name, &par);                  /* attrib do IDNew() no par, tipo no fwd */
		if (fwd > 40) yyerror("duplicate function");   /* if > 40 duplicate function */
		else IDreplace(fwd+40, name, par);			   /* else mark function as used */
	}
}
